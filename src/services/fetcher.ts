...
export const cmcFetcher = async (url: string) => {
...
export const ccFetcher = (url: string) => fetch(url, { headers: { 'authorization': `Apikey ${secrets.CC_KEY}` } }).then(r => r.json().then(res => res.Data))
